...
const PARTY_TYPES_KEY = "party_types_v3"; // ← same key as SystemSettings
...
const CustomerDatabase = () => {
  ...
  // Read-only party types — managed in System Settings
  ...
  // Load party types from storage (set by System Settings)
  useEffect(() => {
    const loadPartyTypes = () => {
      try {
        const stored = localStorage.getItem(PARTY_TYPES_KEY);
        if (stored) {
          const parsed = JSON.parse(stored);
          if (parsed.length > 0) return parsed;
        }
      } catch {}
      return [{ id: 1, name: "OEM" }, { id: 2, name: "End Customer" }];
    };

    const types = loadPartyTypes();
    setPartyTypes(types);
  ...
  const startUpload = (file) => {
    ...
    const timer = setInterval(() => {
      setUploadProgress((prev) => {
        if (prev >= 90) { clearInterval(timer); processFile(file); return 90; }
        return prev + 15;
      });
    }, 100);
  };
  ...
  const processFile = (file) => {
    ...
    reader.onload = (evt) => {
      ...
      if (!rawData || rawData.length === 0) { alert("Empty File"); setUploadStep(1); return; }

      let headerIdx = -1;
      for (let i = 0; i < Math.min(rawData.length, 20); i++) {
        if (rawData[i].some((cell) => cleanStr(cell).includes("partycode"))) { headerIdx = i; break; }
      }
      if (headerIdx === -1) { alert("Could not find 'Party Code' column."); setUploadStep(1); return; }
      ...
      const idxPartyDesc        = getColIndex(["party", "desc"]) !== -1 ? getColIndex(["party", "desc"]) : getColIndex(["party", "name"]);
      ...
      const idxDistrictCity     = getColIndex(["district"]) !== -1 ? getColIndex(["district"]) : getColIndex(["city"]);
      ...
      const processed = rawData.slice(headerIdx + 1).map((row) => {
        ...
        return {
          partyCode:         idxPartyCode       !== -1 ? String(row[idxPartyCode]).trim()       : "",
          partyDescription:  idxPartyDesc       !== -1 ? String(row[idxPartyDesc]).trim()       : "",
          partyType: pType,
          state:             idxState           !== -1 ? String(row[idxState]).trim()           : "",
          districtCity:      idxDistrictCity    !== -1 ? String(row[idxDistrictCity]).trim()    : "",
          productSegment:    idxProductSegment  !== -1 ? row[idxProductSegment]                 : "",
          itemCode:          idxItemCode        !== -1 ? row[idxItemCode]                       : "",
          itemDescription:   idxItemDesc        !== -1 ? row[idxItemDesc]                       : "",
          warrantyPeriodDays:idxWarrantyPeriod  !== -1 ? row[idxWarrantyPeriod]                 : "",
        };
      });
  ...
  const validateData = (rows) => {
    ...
    rows.forEach((row, idx) => {
      ...
      const rowNum = idx + 2;
      if (!row.partyCode) { issues.push({ id: `err-p-${idx}`, row: rowNum, type: "error", message: "Missing Party Code" }); return; }
      if (!row.itemCode)  { issues.push({ id: `err-i-${idx}`, row: rowNum, type: "error", message: "Missing Item Code"  }); return; }
      const clean = cleanStr(row.itemCode);
      if (existingItemCodes.has(clean)) { issues.push({ id: `dup-db-${idx}`,   row: rowNum, type: "duplicate", message: `Item Code "${row.itemCode}" exists in DB`        }); return; }
      if (fileItemCodes.has(clean))     { issues.push({ id: `dup-file-${idx}`, row: rowNum, type: "duplicate", message: `Duplicate Item "${row.itemCode}" in file`         }); return; }
    ...
    setValidationResult({
      total: rows.length, valid: validCount,
  ...
  const commitData = () => { saveToStorage([...validationResult.validRows, ...data]); setUploadStep(4); };

  const resetUpload = () => {
    setShowUploadModal(false);
    setTimeout(() => { setUploadStep(1); setUploadProgress(0); setValidationResult(null); }, 300);
  };
  ...
  const handleDelete = (itemCode) => {
    if (confirm("Delete this item?")) {
      ...
      const s = new Set(selectedItems); s.delete(itemCode); setSelectedItems(s);
    }
  };
  ...
  const filteredData = useMemo(
    () => data.filter((item) => {
      ...
      const matchesType = filterType === "All" || String(item.partyType).toLowerCase() === filterType.toLowerCase();
  ...
  const isPageSelected = paginatedData.length > 0 && paginatedData.every((item) => selectedItems.has(item.itemCode));
      ...
      <div className="flex flex-col gap-[1.5vw] mb-[0.9vw]">
        <div className="flex items-center justify-between bg-white p-[0.7vw] rounded-[0.6vw] shadow-sm border border-gray-200">
          ...
          <div className="flex gap-[1vw] items-center">
            ...
            {/* ── No more "Categories" button here — managed in System Settings ── */}
      ...
      <div className="bg-white rounded-[0.6vw] shadow-sm border border-gray-200 flex flex-col">
            ...
            <thead className="bg-blue-50 sticky top-0 z-10 shadow-sm">
              <tr>
                <th className="p-[0.6vw] border-b border-r border-gray-200 w-[3%] text-center">
                  <button onClick={() => handleSelectAllPage(paginatedData)} className="flex items-center justify-center w-full cursor-pointer">
                    {isPageSelected ? <CheckSquare className="w-[1.1vw] h-[1.1vw] text-blue-600" /> : <Square className="w-[1.1vw] h-[1.1vw] text-gray-400" />}
                  </button>
                </th>
                <th className="p-[0.6vw] font-semibold text-gray-800 border-b border-r border-gray-200 w-[4%] text-center">S.No</th>
                <th className="p-[0.6vw] font-semibold text-gray-800 border-b border-r border-gray-200">Party Type</th>
                <th className="p-[0.6vw] font-semibold text-gray-800 border-b border-r border-gray-200">Product Segment</th>
                <th className="p-[0.6vw] font-semibold text-gray-800 border-b border-r border-gray-200">Party Code</th>
                <th className="p-[0.6vw] font-semibold text-gray-800 border-b border-r border-gray-200">Party Description</th>
                <th className="p-[0.6vw] font-semibold text-gray-800 border-b border-r border-gray-200">Item Code</th>
                <th className="p-[0.6vw] font-semibold text-gray-800 border-b border-r border-gray-200">Item Description</th>
                <th className="p-[0.6vw] font-semibold text-gray-800 border-b border-r border-gray-200">Warranty (days)</th>
                <th className="p-[0.6vw] font-semibold text-gray-800 border-b border-r border-gray-200">State</th>
                <th className="p-[0.6vw] font-semibold text-gray-800 border-b border-gray-200">District/City</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {paginatedData.length > 0 ? (
                paginatedData.map((row, i) => {
                  ...
                  return (
                    <tr key={i} className={`transition-colors ${isSelected ? "bg-blue-50" : "hover:bg-gray-50"}`}>
                      <td className="p-[1vw] border-r border-gray-200 text-center">
                        <button onClick={() => handleSelectItem(row.itemCode)} className="flex items-center justify-center w-full cursor-pointer">
                      ...
                      <td className="p-[0.9vw] text-gray-600 font-medium border-r border-gray-200 text-center">{serialNumber}</td>
                      {!isSameParty && (
                        <td rowSpan={rowSpan} className="p-[0.9vw] border-r border-gray-200 bg-white align-middle">
                          <span className={`px-2 py-1 inline-block min-w-[6vw] text-center rounded text-[0.7vw] font-medium ${getTypeColor(row.partyType)}`}>
                            {row.partyType}
                          </span>
                        </td>
                      )}
                      <td className="p-[0.9vw] text-gray-700 border-r border-gray-200">{row.productSegment}</td>
                      {!isSameParty && (
                        <td rowSpan={rowSpan} className="p-[0.9vw] text-gray-800 font-semibold border-r border-gray-200 bg-white align-middle">{row.partyCode}</td>
                      )}
                      {!isSameParty && (
                        <td rowSpan={rowSpan} className="p-[0.9vw] text-gray-700 border-r border-gray-200 bg-white align-middle truncate max-w-[14vw]" title={row.partyDescription}>{row.partyDescription}</td>
                      )}
                      <td className="p-[0.9vw] text-gray-700 font-mono border-r border-gray-200">{row.itemCode}</td>
                      <td className="p-[0.9vw] text-gray-700 border-r border-gray-200 truncate max-w-[16vw]" title={row.itemDescription}>{row.itemDescription}</td>
                      <td className="p-[0.9vw] text-gray-700 border-r border-gray-200 text-center font-semibold">{row.warrantyPeriodDays}</td>
                      {!isSameParty && (
                        <td rowSpan={rowSpan} className="p-[0.9vw] text-gray-600 border-r border-gray-200 bg-white align-middle">{row.state || "—"}</td>
                      )}
                      {!isSameParty && (
                        <td rowSpan={rowSpan} className="p-[0.9vw] text-gray-600 bg-white align-middle">{row.districtCity || "—"}</td>
                      )}
                    </tr>
                  );
                })
                ...
                <tr>
                  <td colSpan="11" className="py-[4vw] text-center text-gray-400">No data found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="border-t border-blue-100 p-[0.6vw] bg-blue-50 flex justify-between items-center rounded-b-[0.6vw]">
          <div className="text-[0.8vw] text-gray-500">
            Showing <span className="font-semibold text-gray-800">{paginatedData.length > 0 ? (currentPage - 1) * ITEMS_PER_PAGE + 1 : 0}</span> to{" "}
            <span className="font-semibold text-gray-800">{Math.min(currentPage * ITEMS_PER_PAGE, filteredData.length)}</span> of{" "}
            <span className="font-bold text-gray-800">{filteredData.length}</span> entries
          </div>
          <div className="flex items-center gap-[1.2vw]">
            <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-[0.4vw] border border-gray-300 rounded-[0.3vw] hover:bg-white disabled:opacity-50 bg-white shadow-sm cursor-pointer">
            ...
            <div className="flex gap-[0.7vw]">
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return (
                  <button key={pNum} onClick={() => setCurrentPage(pNum)} className={`w-[1.8vw] h-[1.8vw] flex items-center justify-center rounded-[0.3vw] text-[0.8vw] font-medium cursor-pointer ${currentPage === pNum ? "bg-blue-600 text-white" : "bg-white border border-gray-300 text-gray-600 hover:bg-gray-50"}`}>
                    {pNum}
                  </button>
                );
              })}
            </div>
            <button onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages || totalPages === 0} className="p-[0.4vw] border border-gray-300 rounded-[0.3vw] hover:bg-white disabled:opacity-50 bg-white shadow-sm cursor-pointer">
              ...
              <form onSubmit={handleManualSubmit} className="p-[1vw] flex flex-col gap-[1vw] overflow-y-auto">
                <div className="bg-gray-50 p-[1vw] rounded-[0.5vw] border border-gray-200">
                  ...
                  <div className="grid grid-cols-2 gap-[1.5vw] mb-[0.8vw]">
                    <div className="flex flex-col gap-[0.4vw]">
                      ...
                      <input required value={newEntry.partyCode} onChange={(e) => setNewEntry({ ...newEntry, partyCode: e.target.value })} className="border p-[0.6vw] rounded-[0.4vw] bg-white focus:ring-2 ring-blue-100 outline-none" placeholder="e.g. CUS-001" />
                    </div>
                    <div className="flex flex-col gap-[0.4vw]">
                      <label className="text-gray-600 font-medium">Party Type</label>
                      <select value={newEntry.partyType} onChange={(e) => setNewEntry({ ...newEntry, partyType: e.target.value })} className="border p-[0.6vw] rounded-[0.4vw] bg-white outline-none">
                        {partyTypes.map((type) => <option key={type.id} value={type.name}>{type.name}</option>)}
                      </select>
                    </div>
                  </div>
                  <div className="flex flex-col gap-[0.4vw] mb-[0.8vw]">
                    <label className="text-gray-600 font-medium">Party Description *</label>
                    <input required value={newEntry.partyDescription} onChange={(e) => setNewEntry({ ...newEntry, partyDescription: e.target.value })} className="border p-[0.6vw] rounded-[0.4vw] bg-white focus:ring-2 ring-blue-100 outline-none" placeholder="Company Name" />
                  </div>
                  <div className="grid grid-cols-2 gap-[1.5vw]">
                    <div className="flex flex-col gap-[0.4vw]">
                      <label className="text-gray-600 font-medium">State</label>
                      <input value={newEntry.state} onChange={(e) => setNewEntry({ ...newEntry, state: e.target.value })} className="border p-[0.6vw] rounded-[0.4vw] bg-white focus:ring-2 ring-blue-100 outline-none" placeholder="e.g. Tamil Nadu" />
                    </div>
                    <div className="flex flex-col gap-[0.4vw]">
                      <label className="text-gray-600 font-medium">District / City</label>
                      <input value={newEntry.districtCity} onChange={(e) => setNewEntry({ ...newEntry, districtCity: e.target.value })} className="border p-[0.6vw] rounded-[0.4vw] bg-white focus:ring-2 ring-blue-100 outline-none" placeholder="e.g. Coimbatore" />
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-[0.5vw] p-[1vw]">
                  <div className="flex justify-between items-center mb-[0.5vw]">
                    ...
                    <button type="button" onClick={handleAddItemRow} className="text-blue-600 hover:text-blue-800 text-[0.8vw] font-semibold flex items-center gap-1 cursor-pointer">
                      <PlusCircle className="w-[1vw] h-[1vw]" /> Add Item
                    </button>
                  </div>
                  <div className="space-y-[0.8vw]">
                    {newEntry.items.map((item, idx) => (
                      <div key={idx} className="flex gap-[0.5vw] items-center border-b border-gray-100 pb-[0.5vw] last:border-0">
                        <div className="flex-1 flex flex-col gap-[0.3vw]">
                          {idx === 0 && <label className="text-[0.75vw] text-gray-500">Product Segment</label>}
                          <input required value={item.productSegment} onChange={(e) => handleItemChange(idx, "productSegment", e.target.value)} className="border p-[0.5vw] rounded-[0.3vw] text-[0.85vw]" placeholder="Segment" />
                        </div>
                        <div className="flex-1 flex flex-col gap-[0.3vw]">
                          {idx === 0 && <label className="text-[0.75vw] text-gray-500">Item Code</label>}
                          <input required value={item.itemCode} onChange={(e) => handleItemChange(idx, "itemCode", e.target.value)} className="border p-[0.5vw] rounded-[0.3vw] text-[0.85vw]" placeholder="Code" />
                        </div>
                        <div className="flex-[2] flex flex-col gap-[0.3vw]">
                          {idx === 0 && <label className="text-[0.75vw] text-gray-500">Description</label>}
                          <input required value={item.itemDescription} onChange={(e) => handleItemChange(idx, "itemDescription", e.target.value)} className="border p-[0.5vw] rounded-[0.3vw] text-[0.85vw]" placeholder="Description" />
                        </div>
                        <div className="flex-[0.7] flex flex-col gap-[0.3vw]">
                          {idx === 0 && <label className="text-[0.75vw] text-gray-500">Warranty (days)</label>}
                          <input required type="number" value={item.warrantyPeriodDays} onChange={(e) => handleItemChange(idx, "warrantyPeriodDays", e.target.value)} className="border p-[0.5vw] rounded-[0.3vw] text-[0.85vw]" placeholder="Days" />
                        </div>
                        <div className="flex flex-col justify-end">
                          <button type="button" onClick={() => handleRemoveItemRow(idx)} disabled={newEntry.items.length === 1} className={`text-red-400 hover:text-red-600 disabled:opacity-30 cursor-pointer ${idx === 0 ? "mt-[1.3vw]" : ""}`}>
                ...
                <div className="flex justify-end gap-[1vw] pt-[0.5vw]">
                  <button type="button" onClick={() => setShowAddModal(false)} className="px-[2vw] py-[0.6vw] border rounded-[0.4vw] hover:bg-gray-50 cursor-pointer">Cancel</button>
                  <button type="submit" className="px-[2vw] py-[0.6vw] bg-black text-white rounded-[0.4vw] hover:bg-gray-800 flex items-center gap-[0.5vw] cursor-pointer">Save All</button>
                </div>
              </form>
            </motion.div>
          </div>
              ...
              <div className="px-[2vw] py-[1.2vw] flex-1 flex flex-col justify-center min-h-[25vw]">
                {uploadStep === 1 && (
                  ...
                  >
                    ...
                    <p className="text-[1.1vw] font-semibold text-gray-800 mb-[0.4vw]">{isDragging ? "Drop file here" : "Click to upload or Drag & Drop"}</p>
                  ...
                  <div className="flex gap-[2vw] h-full items-stretch">
                    <div className="w-[18vw] flex flex-col gap-[1vw]">
                      <div className="bg-gray-50 p-[1.5vw] rounded-[0.6vw] border border-gray-200 space-y-[0.8vw]">
                        ...
                        <div className="flex justify-between"><span className="text-gray-500">Total</span><span className="font-bold">{validationResult.total}</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Valid</span><span className="font-bold text-green-600">{validationResult.valid}</span></div>
                        <div className="flex justify-between"><span className="text-gray-500">Errors</span><span className="font-bold text-red-600">{validationResult.errors}</span></div>
                      </div>
                    </div>
                      ...
                      <div className="overflow-y-auto flex-1 p-[1vw] space-y-[0.8vw] max-h-[20vw] bg-white">
                        {validationResult.issues.length > 0 ? (
                          validationResult.issues.map((issue, idx) => (
                            <div key={idx} className={`flex items-start gap-[0.8vw] p-[0.8vw] rounded-[0.4vw] border ${issue.type === "error" ? "bg-red-50 border-red-100" : "bg-blue-50 border-orange-100"}`}>
                              <div>
                                <p className={`font-bold ${issue.type === "error" ? "text-red-700" : "text-blue-500"}`}>Row {issue.row}: {issue.type === "error" ? "Error" : "Duplicate"}</p>
                ...
                {uploadStep === 4 && (
                  <div className="flex flex-col items-center justify-center h-full w-full">
                    ...
                    <button onClick={resetUpload} className="px-[3vw] py-[0.8vw] text-[0.9vw] font-medium border border-gray-300 rounded-[0.4vw] hover:bg-gray-50 cursor-pointer">Exit</button>
                  </div>
                )}
              </div>
              {uploadStep === 3 && (
                <div className="px-[1.5vw] py-[1vw] border-t border-gray-200 bg-gray-50 flex justify-end gap-[1vw]">
                  <button onClick={resetUpload} className="px-[2vw] py-[0.6vw] border border-gray-300 rounded-[0.4vw] bg-white hover:bg-gray-50 cursor-pointer">Cancel</button>
                  <button onClick={commitData} disabled={validationResult.valid === 0} className={`px-[1vw] py-[0.6vw] rounded-[0.4vw] text-white flex items-center cursor-pointer gap-[0.5vw] ${validationResult.valid > 0 ? "bg-blue-600 hover:bg-blue-700" : "bg-gray-400 cursor-not-allowed"}`}>
