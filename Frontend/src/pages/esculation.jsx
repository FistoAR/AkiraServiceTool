...
const AssignVisitModal = ({ type, entry, product, currentUser, onSave, onClose, inlineMode = false }) => {
  ...
  const formBody = (
    <div className="flex flex-col gap-[0.9vw]">
      ...
      {/* Contact person info from entry */}
      {(entry?.contactPerson || entry?.contactNumber) && (
        <div className="bg-gray-50 border border-gray-200 rounded-[0.4vw] p-[0.5vw] flex items-center gap-[1vw]">
          <div className="flex items-center gap-[0.3vw] text-[0.72vw] text-gray-600">
            <User className="w-[0.8vw] h-[0.8vw] text-gray-400" />
            <span className="font-semibold">Contact:</span> {entry.contactPerson || "—"}
          </div>
          {entry.contactNumber && (
            <div className="flex items-center gap-[0.3vw] text-[0.72vw] text-gray-600">
              <span className="font-semibold">Ph:</span> {entry.contactNumber}
            </div>
          )}
          {entry.emailId && (
            <div className="flex items-center gap-[0.3vw] text-[0.72vw] text-gray-600">
              <span className="font-semibold">Email:</span> {entry.emailId}
            </div>
          )}
          {entry.location && (
            <div className="flex items-center gap-[0.3vw] text-[0.72vw] text-gray-600">
              <MapPin className="w-[0.8vw] h-[0.8vw] text-gray-400" />
              {entry.location}
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 gap-[0.8vw]">
        ...
        <div className="flex flex-col gap-[0.3vw]">
          <label className="text-[0.8vw] font-semibold text-gray-600">{isFV ? "Visit Date" : "Received Date"}</label>
      ...
      <div className="grid grid-cols-2 gap-[0.8vw]">
        <div className="flex flex-col gap-[0.3vw]">
          <label className="text-[0.8vw] font-semibold text-gray-600">Spare Required?</label>
          <div className="flex gap-[0.5vw]">
            {["Yes", "No"].map(o => (
              <button key={o} type="button" onClick={() => sf("spareRequired", o)}
                className={`flex-1 py-[0.5vw] rounded-[0.4vw] border text-[0.82vw] font-medium cursor-pointer transition-all ${form.spareRequired === o ? (isFV ? "bg-blue-600 text-white border-blue-600" : "bg-purple-600 text-white border-purple-600") : "bg-white border-gray-300 text-gray-600"}`}>
                {o}
              </button>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-[0.3vw]">
          <label className="text-[0.8vw] font-semibold text-gray-600">Spare Details</label>
          <input value={form.spareUsedDetails} onChange={e => sf("spareUsedDetails", e.target.value)}
            ...
            className="border border-gray-300 rounded-[0.4vw] p-[0.6vw] text-[0.82vw] outline-none disabled:bg-gray-100 disabled:text-gray-400" />
        </div>
      </div>
    </div>
  );
  ...
  if (inlineMode) return (
    <div className={`border ${isFV ? "border-blue-200" : "border-purple-200"} rounded-[0.5vw] overflow-hidden bg-white`}>
      <div className={`${isFV ? "bg-blue-50 border-b border-blue-200" : "bg-purple-50 border-b border-purple-200"} px-[0.8vw] py-[0.5vw] flex items-center gap-[0.4vw]`}>
        {isFV ? <MapPin className="w-[0.9vw] h-[0.9vw] text-blue-600" /> : <Wrench className="w-[0.9vw] h-[0.9vw] text-purple-600" />}
        <span className={`text-[0.82vw] font-bold ${isFV ? "text-blue-800" : "text-purple-800"}`}>Assign {type}</span>
      </div>
...
// hidePending: true when inside the Pending tab (product is already pending, no point showing it again)
const ProductClosurePanel = ({ prod, prodIdx, entry, currentUser, onAssignFieldVisit, onAssignInhouse, onProductClose, hidePending = false }) => {
  ...
  if (saved === "Closed") return (
    <div className="mt-[0.5vw] bg-green-50 border border-green-200 rounded-[0.4vw] px-[0.7vw] py-[0.4vw] flex items-center gap-[0.5vw]">
      ...
      <span className="text-[0.72vw] font-bold text-green-700">
        Closed via {existing.closureType === "FieldVisit" ? "Field Visit" : existing.closureType === "InhouseRepair" ? "In-house Repair" : existing.resolutionType || "Direct"}
      </span>
      {existing.remarks && <span className="text-[0.68vw] text-green-600 truncate">· {existing.remarks}</span>}
    </div>
  );

  // Build action tabs — omit Pending when hidePending is true
  const actionTabs = [
    { key: "Closed",  label: "Close Product", emoji: "✓", activeCls: "bg-green-600 text-white",  hoverCls: "hover:bg-green-50 text-gray-600"  },
    { key: "Open",    label: "Assign Work",   emoji: "↗", activeCls: "bg-blue-600 text-white",   hoverCls: "hover:bg-blue-50 text-gray-600"   },
    ...(!hidePending ? [{ key: "Pending", label: "Mark Pending", emoji: "⏸", activeCls: "bg-yellow-500 text-white", hoverCls: "hover:bg-yellow-50 text-gray-600" }] : []),
  ];

  return (
    <>
      <div className="mt-[0.6vw] rounded-[0.5vw] border border-gray-200 overflow-hidden bg-white">
        {/* Action tab buttons */}
        <div className={`grid divide-x divide-gray-200`} style={{ gridTemplateColumns: `repeat(${actionTabs.length}, 1fr)` }}>
          {actionTabs.map(({ key, label, emoji, activeCls, hoverCls }) => (
            <button key={key} type="button"
              onClick={() => setSelected(selected === key ? "" : key)}
              className={`py-[0.55vw] text-[0.72vw] font-semibold cursor-pointer transition-all flex items-center justify-center gap-[0.3vw] border-b border-gray-200 ${selected === key ? activeCls : `bg-gray-50 ${hoverCls}`}`}>
              <span className="text-[0.8vw]">{emoji}</span>
              <span>{label}</span>
            </button>
          ))}
        </div>
        ...
        {/* ── Closed sub-panel ── */}
        {selected === "Closed" && (
          <div className="p-[0.65vw] space-y-[0.4vw]">
            ...
            <div className="grid grid-cols-2 gap-[0.3vw]">
              {["Fixed", "Replaced", "No Fault Found", "Partially Fixed"].map(t => (
                <button key={t} type="button" onClick={() => setResolutionType(t)}
                  className={`py-[0.35vw] rounded-[0.3vw] border text-[0.68vw] font-medium cursor-pointer transition-all ${resolutionType === t ? "bg-green-600 text-white border-green-600" : "bg-white border-gray-200 text-gray-600 hover:border-green-300"}`}>
                  {t}
                </button>
              ))}
            </div>
            <textarea rows="2" value={remarks} onChange={e => setRemarks(e.target.value)}
              ...
              className="w-full border border-gray-200 rounded-[0.3vw] p-[0.4vw] text-[0.7vw] outline-none resize-none focus:border-green-400 bg-gray-50" />
            <button onClick={() => {
        ...
        {/* ── Open/Assign sub-panel ── */}
        {selected === "Open" && (
          <div className="p-[0.65vw]">
            <div className="text-[0.65vw] font-bold text-gray-400 uppercase tracking-wider mb-[0.4vw]">Choose Assignment Type</div>
            <div className="grid grid-cols-2 gap-[0.4vw]">
              <button type="button" onClick={() => setShowFVModal(true)}
                className="flex flex-col items-center gap-[0.3vw] bg-blue-50 hover:bg-blue-100 border border-blue-200 text-blue-700 py-[0.7vw] rounded-[0.4vw] text-[0.72vw] font-semibold cursor-pointer transition-all">
                <MapPin className="w-[1.1vw] h-[1.1vw]" />
                <span>Field Visit</span>
              </button>
              <button type="button" onClick={() => setShowIHModal(true)}
                className="flex flex-col items-center gap-[0.3vw] bg-purple-50 hover:bg-purple-100 border border-purple-200 text-purple-700 py-[0.7vw] rounded-[0.4vw] text-[0.72vw] font-semibold cursor-pointer transition-all">
                <Wrench className="w-[1.1vw] h-[1.1vw]" />
        ...
        {/* ── Pending sub-panel — only when not hidePending ── */}
        {selected === "Pending" && !hidePending && (
          <div className="p-[0.65vw] space-y-[0.35vw]">
            <div className="text-[0.65vw] font-bold text-gray-400 uppercase tracking-wider">Pending Reason</div>
            <textarea rows="2" value={remarks} onChange={e => setRemarks(e.target.value)}
              placeholder="Reason for pending / awaiting parts / notes…"
              className="w-full border border-gray-200 rounded-[0.3vw] p-[0.4vw] text-[0.7vw] outline-none resize-none focus:border-yellow-400 bg-gray-50" />
            <button onClick={() => {
              onProductClose(prodIdx, { status: "Pending", closureType: "Pending", remarks, pendingAt: new Date().toISOString() });
            }} className="w-full py-[0.42vw] bg-yellow-500 hover:bg-yellow-600 text-white rounded-[0.3vw] text-[0.7vw] font-semibold cursor-pointer flex items-center justify-center gap-[0.3vw]">
              <AlertCircle className="w-[0.75vw] h-[0.75vw]" />Mark as Pending
            </button>
          </div>
        )}
      </div>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-[2vw]">
      <div className="bg-white w-[60vw] max-h-[85vh] rounded-[0.8vw] shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 p-[1.5vw] space-y-[1vw]">
          ...
          {/* In-house Repairs */}
          {rec._inhouseRepairs?.length > 0 && (
            <div className="bg-purple-50 border border-purple-200 rounded-[0.5vw] overflow-hidden">
              <div className="bg-purple-100 border-b border-purple-200 px-[0.8vw] py-[0.5vw] flex items-center gap-[0.4vw]">
                <Wrench className="w-[0.9vw] h-[0.9vw] text-purple-600" />
                <span className="text-[0.8vw] font-bold text-purple-700">In-house Repairs</span>
              </div>
              <div className="p-[0.8vw] space-y-[0.4vw]">
                {rec._inhouseRepairs.map((r, i) => (
                  <div key={i} className="bg-white border border-purple-100 rounded-[0.4vw] p-[0.5vw]">
                    <div className="flex justify-between">
                      <span className="text-[0.78vw] font-semibold">Assigned to: {r.assignedToName}</span>
                      <Badge label={r.visitStatus === "Closed" ? "Completed" : "Open"} color={r.visitStatus === "Closed" ? "green" : "purple"} />
                    </div>
                    {r.productIdx !== null && r.productIdx !== undefined && (
                      <div className="text-[0.68vw] text-purple-600 mt-[0.1vw]">Product: P{r.productIdx + 1}</div>
                    )}
                    {r.fieldDiagnosisSummary && <div className="text-[0.72vw] text-gray-600 mt-[0.1vw]">Summary: {r.fieldDiagnosisSummary}</div>}
                    {r.resolutionRemarks && <div className="text-[0.72vw] text-green-700">Resolution: {r.resolutionRemarks}</div>}
                  </div>
                ))}
              </div>
            </div>
    ...
    <div className="mt-[0.5vw]">
      <div className="flex gap-[0.3vw] mb-[0.8vw] bg-white border border-gray-200 rounded-[0.5vw] p-[0.25vw]">
        <button onClick={() => setViewMode("assigned")}
          className={`flex-1 flex items-center justify-center gap-[0.35vw] py-[0.45vw] rounded-[0.35vw] text-[0.78vw] font-semibold cursor-pointer transition-all ${viewMode === "assigned" ? (isFV ? "bg-blue-600" : "bg-purple-600") + " text-white shadow-sm" : "text-gray-500 hover:bg-gray-50"}`}>
          <Icon className="w-[0.85vw] h-[0.85vw]" />Assigned Records
          {mine.length > 0 && (
            <span className={`text-[0.62vw] px-[0.4vw] rounded-full font-bold ${viewMode === "assigned" ? "bg-white " + (isFV ? "text-blue-600" : "text-purple-600") : "bg-gray-200 text-gray-600"}`}>{mine.length}</span>
          )}
        </button>
        ...
        mine.length === 0
          ? (
            <div className="bg-white rounded-[0.5vw] p-[3vw] text-center border border-gray-200">
              ...
              <p className="text-[0.8vw] text-gray-300 mt-[0.3vw]">{type} assignments will appear here</p>
            </div>
          ) : (
            <div className="space-y-[0.8vw]">
              {mine.map(rec => {
                ...
                return (
                  <div key={rec.id} className={`bg-white rounded-[0.6vw] border overflow-hidden hover:shadow-md transition-all ${isClosed ? "border-green-300" : isFV ? "border-blue-200" : "border-purple-200"}`}>
                    <div className="p-[0.9vw] cursor-pointer" onClick={() => setExpanded(isExp ? null : rec.id)}>
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-[0.7vw]">
                          <div className={`w-[2vw] h-[2vw] rounded-full flex items-center justify-center flex-shrink-0 ${isClosed ? "bg-green-500" : isFV ? "bg-blue-500" : "bg-purple-500"}`}>
                          ...
                          <div>
                            <div className="flex items-center gap-[0.4vw]">
                              ...
                              <Badge label={isClosed ? "Completed" : "Open"} color={isClosed ? "green" : isFV ? "blue" : "purple"} />
                              {!isAssignee && <Badge label="Assigned by you" color="gray" />}
                              {rec.productIdx !== null && rec.productIdx !== undefined && (
                                <Badge label={`P${rec.productIdx + 1}`} color={isFV ? "blue" : "purple"} />
                              )}
                            </div>
                            <div className="text-[0.72vw] text-gray-500 mt-[0.15vw]">
                              ...
                              {rec.contactPerson && ` · Contact: ${rec.contactPerson}`}
                    ...
                    {isExp && (
                      <div className="border-t border-gray-100 p-[0.9vw] bg-gray-50">
                        <div className="grid grid-cols-2 gap-[0.4vw] text-[0.75vw] bg-white border border-gray-200 rounded-[0.4vw] p-[0.7vw] mb-[0.6vw]">
                          ...
                          <span className={rec.spareRequired === "Yes" ? "text-orange-600 font-semibold" : "text-gray-600"}>{rec.spareRequired}</span>
                          {rec.diagnosisSummary && <><span className="text-gray-400">Diagnosis:</span><span>{rec.diagnosisSummary}</span></>}
                          {rec.contactPerson && <><span className="text-gray-400">Contact Person:</span><span className="font-medium">{rec.contactPerson}</span></>}
                          {rec.contactNumber && <><span className="text-gray-400">Contact No:</span><span>{rec.contactNumber}</span></>}
                          {rec.emailId && <><span className="text-gray-400">Email:</span><span>{rec.emailId}</span></>}
                          {rec.location && <><span className="text-gray-400">Location:</span><span>{rec.location}</span></>}
                        </div>
        ...
        openCalls.length === 0
          ? (
            ...
            <div className="space-y-[0.6vw]">
              <div className="bg-orange-50 border border-orange-200 rounded-[0.4vw] px-[0.8vw] py-[0.5vw] text-[0.75vw] text-orange-700 font-medium">
                ⚡ These calls are assigned to you but have no {type} scheduled yet. Assign per product below.
              </div>
              {openCalls.map(entry => {
                        ...
                        <div className="flex items-center gap-[0.6vw]">
                          ...
                          <div>
                            <div className="flex items-center gap-[0.4vw]">
                              ...
                              <Badge label={`${entry.products?.length || 0} product(s)`} color="orange" />
                            </div>
                            ...
                            <div key={pIdx} className="bg-white rounded-[0.4vw] border border-gray-200 overflow-hidden">
                              ...
                              <div className="p-[0.6vw]">
                                <AssignVisitModal type={type} entry={entry} product={prod} currentUser={currentUser}
                                  ...
                                  onSave={(form) => {
                                    if (onAssignVisit) onAssignVisit(entry.callId, type, form, pIdx);
...
// ── Support Request Action Panel (full closure for assigned requests) ──────────
const SupportReqActionPanel = ({ req, currentUser, onDone }) => {
  const [action, setAction]         = useState(""); // "close" | "pending" | "fv" | "inhouse" | "escalate"
  const [resolutionType, setResType] = useState("Fixed");
  const [remarks, setRemarks]        = useState("");
  const [showFVForm, setShowFVForm]  = useState(false);
  const [showIHForm, setShowIHForm]  = useState(false);
  const [showEscModal, setShowEscModal] = useState(false);

  // Build a fake entry/product from the req so we can reuse existing modals
  const fakeEntry = {
  ...
  const handleClose = () => {
    ...
    // Also close the product on the escalation queue
    const q = loadQ();
    const upd = q.map(e => {
  ...
  const handlePending = () => {
    ...
    // Mark product as pending in queue too
    const q = loadQ();
    const upd = q.map(e => {
      if (e.callId !== req.callId) return e;
      const products = e.products?.map((p, i) => {
        if (i !== req.productIdx) return p;
        return { ...p, _productClosure: { status: "Pending", closureType: "Pending", remarks, pendingAt: new Date().toISOString() } };
      });
      return { ...e, products };
    });
    saveQ(upd);
  ...
  const handleEscalate = (data) => {
    // Save a new support request to the next person
    const reqs = load(SUPPORT_REQ_KEY, []);
    reqs.push({
      id: Date.now(), callId: req.callId, callNumber: req.callNumber,
      product: fakeProduct, productIdx: req.productIdx,
      requestedById: currentUser?.userId, requestedByName: currentUser?.name,
      supportPerson: data.supportPerson, notes: data.notes,
  ...
  if (isResolved) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-[0.4vw] p-[0.6vw]">
        <p className="text-[0.75vw] font-bold text-green-700 mb-[0.2vw]">
          {req.status === "Escalated Further" ? "Escalated Further" : req.status === "Field Visit Assigned" ? "Field Visit Assigned" : req.status === "In-house Repair Assigned" ? "In-house Repair Assigned" : "Resolved"}
        </p>
        <p className="text-[0.75vw] text-green-800">{req.resolutionNotes || req.pendingReason || `Assigned to: ${req.assignedVisitTo || req.escalatedTo || ""}`}</p>
        {req.resolvedAt && <p className="text-[0.68vw] text-green-500 mt-[0.15vw]">{new Date(req.resolvedAt).toLocaleString()}</p>}
      </div>
    );
  }
  ...
  if (req.status === "Pending") {
    return (
      <div className="space-y-[0.5vw]">
        <div className="bg-yellow-50 border border-yellow-200 rounded-[0.4vw] p-[0.5vw]">
          <p className="text-[0.75vw] font-bold text-yellow-700">Marked as Pending</p>
          {req.pendingReason && <p className="text-[0.72vw] text-yellow-600 mt-[0.1vw]">{req.pendingReason}</p>}
        </div>
        {/* Allow further actions even from pending */}
        <div className="grid grid-cols-3 divide-x divide-gray-200 border border-gray-200 rounded-[0.4vw] overflow-hidden">
          {[
            ...
            { key: "inhouse",  label: "In-house",      emoji: "🔧", cls: "bg-purple-600 text-white", hov: "hover:bg-purple-50 text-gray-600" },
          ].map(({ key, label, emoji, cls, hov }) => (
            <button key={key} type="button" onClick={() => setAction(action === key ? "" : key)}
              className={`py-[0.5vw] text-[0.7vw] font-semibold cursor-pointer flex items-center justify-center gap-[0.25vw] ${action === key ? cls : `bg-gray-50 ${hov}`}`}>
  ...
  return (
    <div className="space-y-[0.5vw]">
      {/* 5-action tab row */}
      <div className="grid divide-x divide-gray-200 border border-gray-200 rounded-[0.4vw] overflow-hidden" style={{ gridTemplateColumns: "repeat(5, 1fr)" }}>
        {[
          { key: "close",    label: "Close",         emoji: "✓",  cls: "bg-green-600 text-white",  hov: "hover:bg-green-50 text-gray-600"   },
          { key: "pending",  label: "Pending",        emoji: "⏸", cls: "bg-yellow-500 text-white", hov: "hover:bg-yellow-50 text-gray-600"  },
          { key: "fv",       label: "Field Visit",    emoji: "📍", cls: "bg-blue-600 text-white",   hov: "hover:bg-blue-50 text-gray-600"    },
          { key: "inhouse",  label: "In-house",       emoji: "🔧", cls: "bg-purple-600 text-white", hov: "hover:bg-purple-50 text-gray-600"  },
          { key: "escalate", label: "Ask Support",    emoji: "↗",  cls: "bg-orange-500 text-white", hov: "hover:bg-orange-50 text-gray-600"  },
        ].map(({ key, label, emoji, cls, hov }) => (
          <button key={key} type="button" onClick={() => setAction(action === key ? "" : key)}
      ...
      {/* Sub-panels */}
      {action === "close" && (
        <_CloseSubPanel resolutionType={resolutionType} setResType={setResType} remarks={remarks} setRemarks={setRemarks} onConfirm={handleClose} />
      )}
      {action === "pending" && (
        <div className="p-[0.6vw] border border-yellow-200 rounded-[0.4vw] bg-yellow-50 space-y-[0.35vw]">
          <div className="text-[0.65vw] font-bold text-gray-400 uppercase">Pending Reason</div>
          <textarea rows="2" value={remarks} onChange={e => setRemarks(e.target.value)}
            placeholder="Awaiting parts / further info…"
            className="w-full border border-yellow-200 rounded-[0.3vw] p-[0.4vw] text-[0.7vw] outline-none resize-none focus:border-yellow-400 bg-white" />
          <button onClick={handlePending}
            className="w-full py-[0.4vw] bg-yellow-500 hover:bg-yellow-600 text-white rounded-[0.3vw] text-[0.7vw] font-semibold cursor-pointer flex items-center justify-center gap-[0.3vw]">
            <AlertCircle className="w-[0.75vw] h-[0.75vw]" />Mark as Pending
          </button>
        </div>
      )}
      ...
      {action === "inhouse" && (
        <AssignVisitModal type="In-house Repair" entry={fakeEntry} product={fakeProduct} currentUser={currentUser}
          inlineMode onClose={() => setAction("")}
          onSave={f => { handleAssignVisit("In-house Repair", f); setAction(""); }} />
      )}
      {action === "escalate" && (
        <SupportEscalationModal product={fakeProduct} entry={{ ...fakeEntry, currentDepartment: currentUser?.department }}
          currentUser={currentUser}
          showAll={true}
          onClose={() => setAction("")}
...
// ── Support Requests Tab — split into Raised / Assigned ───────────────────────
const SupportRequestsTab = ({ currentUser }) => {
  ...
  const DONE_STATUSES = ["Resolved","Field Visit Assigned","In-house Repair Assigned","Escalated Further"];
  ...
  const assignedPending = assignedToMe.filter(r => !DONE_STATUSES.includes(r.status)).length;
  const raisedPending   = raisedByMe.filter(r => !DONE_STATUSES.includes(r.status)).length;
    ...
    <div className="mt-[0.5vw]">
      ...
      <div className="flex gap-[0.3vw] mb-[0.8vw] bg-white border border-gray-200 rounded-[0.5vw] p-[0.25vw]">
        <button onClick={() => setSubTab("assigned")}
          ...
          <HelpCircle className="w-[0.85vw] h-[0.85vw]" />
          Request Assigned
          {assignedToMe.length > 0 && (
            <span className={`text-[0.62vw] px-[0.4vw] rounded-full font-bold ${subTab === "assigned" ? "bg-white text-orange-500" : assignedPending > 0 ? "bg-orange-100 text-orange-600" : "bg-gray-200 text-gray-600"}`}>
              {assignedToMe.length}
            </span>
          )}
        </button>
        <button onClick={() => setSubTab("raised")}
          ...
          Request Raised
          {raisedByMe.length > 0 && (
            <span className={`text-[0.62vw] px-[0.4vw] rounded-full font-bold ${subTab === "raised" ? "bg-white text-blue-600" : raisedPending > 0 ? "bg-blue-100 text-blue-600" : "bg-gray-200 text-gray-600"}`}>
              {raisedByMe.length}
            </span>
          )}
        </button>
      ...
      {activeList.length === 0 ? (
        <div className="bg-white rounded-[0.5vw] p-[3vw] text-center border border-gray-200">
          ...
          <p className="text-[1vw] text-gray-400 font-medium">
            {subTab === "assigned" ? "No support requests assigned to you" : "No support requests raised by you"}
          </p>
          <p className="text-[0.8vw] text-gray-300 mt-[0.3vw]">
            {subTab === "assigned" ? "Requests assigned to you will appear here" : "Requests you raised will appear here"}
          </p>
        </div>
      ) : (
        <div className="space-y-[0.7vw]">
          {activeList.map(req => {
            ...
            const isDone     = ["Resolved","Field Visit Assigned","In-house Repair Assigned","Escalated Further"].includes(req.status);
            const statusColor = isDone ? "border-green-300" : subTab === "assigned" ? "border-orange-300" : "border-blue-300";
            return (
              <div key={req.id} className={`bg-white rounded-[0.6vw] border overflow-hidden hover:shadow-md transition-all ${statusColor}`}>
                <div className="p-[0.9vw] cursor-pointer" onClick={() => setExpanded(isExp ? null : req.id)}>
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-[0.6vw]">
                      <div className={`w-[1.8vw] h-[1.8vw] rounded-full flex items-center justify-center flex-shrink-0 ${isDone ? "bg-green-500" : subTab === "assigned" ? "bg-orange-500" : "bg-blue-500"}`}>
                      ...
                      <div>
                        <div className="flex items-center gap-[0.4vw] flex-wrap">
                          ...
                          {req.productIdx !== null && req.productIdx !== undefined && (
                            <Badge label={`P${req.productIdx + 1}`} color="orange" />
                          )}
                        </div>
                        <div className="text-[0.72vw] text-gray-500 mt-[0.15vw]">
                          {subTab === "assigned"
                          ...
                          {req.contactPerson && ` · Contact: ${req.contactPerson}`}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-[0.4vw]">
                ...
                {isExp && (
                  <div className="border-t border-gray-100 p-[0.9vw] bg-gray-50 space-y-[0.6vw]">
                    {/* Product info */}
                    <div className="bg-white border border-gray-200 rounded-[0.4vw] p-[0.7vw] grid grid-cols-2 gap-[0.3vw] text-[0.75vw]">
                      ...
                      <span className="text-gray-400">Error Code:</span><span className="text-red-600 font-mono">{req.product?.errorCode || "—"}</span>
                      {req.product?.callDescription && <><span className="text-gray-400">Issue:</span><span>{req.product.callDescription}</span></>}
                      {req.contactPerson && <><span className="text-gray-400">Contact:</span><span className="font-medium">{req.contactPerson}{req.contactNumber ? ` · ${req.contactNumber}` : ""}</span></>}
                      {req.emailId       && <><span className="text-gray-400">Email:</span><span>{req.emailId}</span></>}
                      {req.location      && <><span className="text-gray-400">Location:</span><span>{req.location}</span></>}
                    </div>
                    {/* Handover notes */}
                    <div className={`border rounded-[0.4vw] p-[0.6vw] ${subTab === "assigned" ? "bg-orange-50 border-orange-200" : "bg-blue-50 border-blue-200"}`}>
                      <p className={`text-[0.75vw] font-bold mb-[0.2vw] ${subTab === "assigned" ? "text-orange-700" : "text-blue-700"}`}>
                        Handover Notes {subTab === "assigned" ? `(from ${req.requestedByName})` : `(to ${req.supportPerson?.name})`}
                      </p>
                      <p className={`text-[0.75vw] ${subTab === "assigned" ? "text-orange-800" : "text-blue-800"}`}>{req.notes}</p>
                    </div>
                    {/* Action area */}
                    {subTab === "assigned"
                      ? <SupportReqActionPanel req={req} currentUser={currentUser} onDone={reload} />
                      : <div className="bg-gray-50 border border-gray-200 rounded-[0.4vw] p-[0.6vw]">
                          <p className="text-[0.75vw] text-gray-500 italic">
                            {isDone ? `Status: ${req.status}` : `Awaiting action from ${req.supportPerson?.name}`}
                          </p>
                        </div>
                    }
                  </div>
...
const PendingTab = ({ queue, currentUser, onProductClose, onAssignFieldVisit, onAssignInhouse }) => {
  ...
  if (pendingItems.length === 0) return (
    <div className="bg-white rounded-[0.5vw] p-[3vw] text-center border border-gray-200 mt-[1vw]">
      ...
      <p className="text-[0.8vw] text-gray-300 mt-[0.3vw]">Products marked as Pending will appear here</p>
    </div>
  );
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-[0.7vw]">
                  ...
                  <div>
                    <div className="flex items-center gap-[0.4vw] flex-wrap">
                      <span className="font-mono text-[0.88vw] font-bold text-gray-800">{entry.callNumber}</span>
                      <span className="text-[0.65vw] px-[0.4vw] py-[0.1vw] rounded-full border font-semibold bg-yellow-100 text-yellow-700 border-yellow-300">
                        Products Pending
                      </span>
                      <span className="text-[0.63vw] px-[0.4vw] py-[0.08vw] rounded bg-yellow-50 text-yellow-700 border border-yellow-200 font-semibold">
                        {pendingProds.length} product{pendingProds.length > 1 ? "s" : ""}
                      </span>
                    </div>
                    <div className="text-[0.72vw] text-gray-500 mt-[0.12vw]">
                      <User className="w-[0.7vw] h-[0.7vw] inline mr-[0.2vw]" />{entry.customerName} · {entry.mode || "—"}
                      {entry.contactPerson && ` · Contact: ${entry.contactPerson}`}
                      {entry.contactNumber && ` (${entry.contactNumber})`}
                    </div>
                  </div>
                </div>
                ...
                {pendingProds.map((prod) => {
                  ...
                  return (
                    <div key={realIdx} className="rounded-[0.4vw] border border-yellow-200 bg-yellow-50/40 p-[0.6vw]">
                      <div className="flex items-start justify-between mb-[0.35vw]">
                        <div className="flex items-center gap-[0.4vw]">
                          <div className="w-[1.4vw] h-[1.4vw] rounded-full bg-yellow-500 flex items-center justify-center text-[0.58vw] font-bold text-white flex-shrink-0">
                            {realIdx + 1}
                          </div>
                          <div>
                            <div className="text-[0.8vw] font-bold text-gray-700">{prod.productModel || prod.itemCode}</div>
                            {prod.serialNumber && <div className="text-[0.65vw] text-gray-400 font-mono">SN: {prod.serialNumber}</div>}
                          </div>
                        </div>
                        <Badge label="Pending" color="yellow" />
                      </div>
                      {prod._productClosure?.remarks && (
                        <div className="text-[0.68vw] text-yellow-700 bg-yellow-50 border border-yellow-100 rounded-[0.3vw] px-[0.5vw] py-[0.2vw] mb-[0.35vw]">
                          <strong>Note:</strong> {prod._productClosure.remarks}
                        </div>
                      )}
                      {/* hidePending=true: product already pending, no point marking pending again */}
                      <ProductClosurePanel
                        prod={prod} prodIdx={realIdx} entry={entry} currentUser={currentUser}
                        hidePending={true}
...
const ReportsTab = ({ currentUser }) => {
  ...
  const allRecords = useMemo(() => {
    ...
    const inhouse  = load(INHOUSE_REPAIR_KEY, []);
    const seen = new Set(); const result = [];
    q.forEach(e => {
      const wasInvolved =
        e.currentEngineerId === uid ||
        e.escalationHistory?.some(h => h.engineerId === uid) ||
        e.products?.some(p => p._escalationHistory?.some(h => h.engineerId === uid)) ||
        fvs.some(f => f.callId === e.callId && (f.assignedTo === uid || f.assignedBy === uid)) ||
        inhouse.some(f => f.callId === e.callId && (f.assignedTo === uid || f.assignedBy === uid)) ||
      ...
      if (wasInvolved && !seen.has(e.callId)) {
        ...
        result.push({
          ...
          _inhouseRepairs: inhouse.filter(f => f.callId === e.callId),
  ...
  const filteredRecords = useMemo(() => allRecords.filter(r => {
    ...
    const q = search.toLowerCase();
    const matchSearch = !q || r.callNumber?.toLowerCase().includes(q) || r.customerName?.toLowerCase().includes(q) || r.currentEngineerName?.toLowerCase().includes(q);
  ...
  const statusTabs = ["All","Assigned","Escalated","Resolved","Pending","Closed","Critical"];
  ...
  return (
    <div className="mt-[0.5vw]">
      <div className="flex items-center gap-[0.8vw] mb-[0.8vw]">
        <input type="text" value={search} onChange={e => setSearch(e.target.value)}
          placeholder="Search by customer, call no, engineer…"
      ...
      <div className="flex gap-[0.3vw] mb-[0.8vw] flex-wrap">
        {statusTabs.map(s => {
          ...
          const colorMap = { All:"blue", Assigned:"blue", Escalated:"orange", Resolved:"green", Pending:"yellow", Closed:"gray", Critical:"red" };
          const activeClsMap = { blue:"bg-blue-600 text-white border-blue-600", orange:"bg-orange-500 text-white border-orange-500", green:"bg-green-600 text-white border-green-600", yellow:"bg-yellow-500 text-white border-yellow-500", gray:"bg-gray-600 text-white border-gray-600", red:"bg-red-600 text-white border-red-600" };
          const dotMap   = { blue:"bg-blue-400", orange:"bg-orange-400", green:"bg-green-400", yellow:"bg-yellow-400", gray:"bg-gray-400", red:"bg-red-400" };
          const c = colorMap[s] || "gray";
          return (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`flex items-center gap-[0.3vw] px-[0.8vw] py-[0.3vw] rounded-full border text-[0.75vw] font-semibold cursor-pointer transition-all ${isActive ? activeClsMap[c] : "bg-white text-gray-600 border-gray-300 hover:border-gray-400"}`}>
              <span className={`w-[0.6vw] h-[0.6vw] rounded-full inline-block ${isActive ? "bg-white" : dotMap[c]}`} />
              {s} <span className={`ml-[0.1vw] text-[0.65vw] font-bold ${isActive ? "text-white/80" : "text-gray-400"}`}>{count}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="bg-white rounded-[0.5vw] border border-gray-200 overflow-hidden shadow-sm">
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              {["S.No","Call No","Date","Customer","Contact","Priority","Engineer","Products","Status","View"].map(h => (
            ...
            {filteredRecords.map((rec, idx) => {
              ...
              return (
                <tr key={rec.callId} className="hover:bg-blue-50/30 transition-colors">
                  <td className="px-[0.7vw] py-[0.6vw] text-[0.75vw] text-gray-500">{idx + 1}</td>
                  <td className="px-[0.7vw] py-[0.6vw]"><span className="text-[0.78vw] font-bold text-blue-600 font-mono">{rec.callNumber}</span></td>
                  <td className="px-[0.7vw] py-[0.6vw] text-[0.72vw] text-gray-600 whitespace-nowrap">
                    {rec.assignedAt ? new Date(rec.assignedAt).toLocaleDateString("en-GB") : "—"}
                  </td>
                  <td className="px-[0.7vw] py-[0.6vw]">
                    <span className="text-[0.78vw] font-bold text-gray-800">{rec.customerName}</span>
                    {rec.partyCode && <div className="text-[0.65vw] text-gray-400">{rec.partyCode}</div>}
                  </td>
                  <td className="px-[0.7vw] py-[0.6vw] text-[0.72vw] text-gray-700">
                  ...
                  <td className="px-[0.7vw] py-[0.6vw]">
                    {rec.priority && <span className={`text-[0.68vw] px-[0.4vw] py-[0.12vw] rounded-full border font-semibold ${getPrioColor(rec.priority)}`}>{rec.priority}</span>}
                  </td>
                  <td className="px-[0.7vw] py-[0.6vw] text-[0.75vw] text-gray-700">{rec.currentEngineerName || "—"}</td>
                  <td className="px-[0.7vw] py-[0.6vw]">
                    <div className="flex items-center gap-[0.25vw] flex-wrap">
                      {rec.products?.map((p, i) => {
                        const cfg = PROD_STATUS_CFG[getProductStatus(p)];
                        return (
                          <div key={i} title={`P${i+1}: ${cfg.label}`}
                            className={`w-[1.4vw] h-[1.4vw] rounded-full flex items-center justify-center text-white text-[0.55vw] font-bold ${cfg.dot}`}>
                            {i+1}
                          </div>
                        );
                      })}
                      <span className="text-[0.65vw] text-gray-400 ml-[0.2vw]">{resolvedCount}/{totalProducts}</span>
                    </div>
                  </td>
                  <td className="px-[0.7vw] py-[0.6vw]">
                    <span className={`text-[0.68vw] px-[0.45vw] py-[0.12vw] rounded border font-semibold ${getStatusBadge(rec.status)}`}>
                      {rec.status === "Critical_Unresolved" ? "CRITICAL" : rec.status}
                    </span>
                  </td>
                  <td className="px-[0.7vw] py-[0.6vw]">
                    <button onClick={() => setDetailRec(rec)}
                      className="flex items-center gap-[0.3vw] bg-blue-600 hover:bg-blue-700 text-white px-[0.7vw] py-[0.35vw] rounded-[0.35vw] text-[0.72vw] font-semibold cursor-pointer whitespace-nowrap">
                      <Eye className="w-[0.75vw] h-[0.75vw]" />Details
                    </button>
                  </td>
                </tr>
              );
        ...
        {filteredRecords.length === 0 && (
          <div className="text-center py-[2vw] text-gray-400 text-[0.82vw]">No records match the current filter</div>
        )}
        <div className="bg-gray-50 border-t border-gray-200 px-[0.8vw] py-[0.5vw]">
          <span className="text-[0.72vw] text-gray-500">
            Showing <strong>{filteredRecords.length}</strong> of <strong>{allRecords.length}</strong> entries
          </span>
        </div>
      </div>
...
const EscalationPage = () => {
  ...
  // A call leaves escalation tab when ALL products are handled:
  // resolved, closed, pending (→ Pending tab), or support-requested (→ Support tab)
  const isMovedOut = (e) =>
    e.products?.length > 0 &&
    e.products.every(p =>
      p._resolved ||
  ...
  const liveCounts = useMemo(() => ({
    ...
    pending:    queue.filter(e =>
      e.currentEngineerId === loggedInUser?.userId &&
      e.products?.some(p => p._productClosure?.status === "Pending")
    ).length,
  }), [queue, loggedInUser]);
  ...
  // ── Assign field visit / inhouse for specific product ──────────────────────
  ...
  const tabs = [
    ...
    { id: "inhouse",    label: "In-house Repair", icon: Wrench,    color: "purple", count: liveCounts.inhouse },
      ...
      <div className="flex-1 max-h-[82vh] overflow-y-auto pr-[0.3vw]">
        ...
        {activeTab === "pending" && (
          <PendingTab
            ...
            onAssignInhouse={(callId, form, pIdx)    => handleAssignVisit(callId, "In-house Repair", form, pIdx)}
          />
        )}
        ...
        {activeTab === "inhouse"    && <VisitsTab type="In-house Repair" currentUser={loggedInUser} onAssignVisit={(callId, type, form, pIdx) => handleAssignVisit(callId, type, form, pIdx)} />}
