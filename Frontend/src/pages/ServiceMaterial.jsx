    ...
    if (isEdit) {
      ...
      const rows = products.map(prod => {
        ...
        const row = {
          ...
          escalationLevel: 0,
          escalationHistory,
          currentEngineerId: base.assignedTo,
          qcActions: [],
        };
        return row;
      });
        ...
        <div className="bg-white rounded-[0.6vw] shadow-sm border border-gray-200 p-[1.2vw]">
          ...
          <div className="grid grid-cols-4 gap-[1.2vw]">
            ...
            <div className="flex flex-col gap-[0.3vw]">
              ...
              <input value={base.contactPerson} onChange={e => sb("contactPerson", e.target.value)} className="border border-gray-300 p-[0.6vw] rounded-[0.4vw] outline-none focus:ring-2 ring-blue-100" />
            </div>

            {/* Contact Number */}
            <div className="flex flex-col gap-[0.3vw]">
              <label className="font-semibold text-gray-600">Contact Number</label>
              <input value={base.contactNumber} onChange={e => sb("contactNumber", e.target.value)} className="border border-gray-300 p-[0.6vw] rounded-[0.4vw] outline-none focus:ring-2 ring-blue-100" />
            </div>

            {/* Email */}
            <div className="flex flex-col gap-[0.3vw]">
              <label className="font-semibold text-gray-600">Email</label>
              <input type="email" value={base.emailId} onChange={e => sb("emailId", e.target.value)} className="border border-gray-300 p-[0.6vw] rounded-[0.4vw] outline-none focus:ring-2 ring-blue-100" />
            </div>

            {/* Location */}
            <div className="col-span-2 flex flex-col gap-[0.3vw]">
              <label className="font-semibold text-gray-600">Location / Site</label>
              <input value={base.location} onChange={e => sb("location", e.target.value)} className="border border-gray-300 p-[0.6vw] rounded-[0.4vw] outline-none focus:ring-2 ring-blue-100" />
            </div>

            {/* ── NEW: Reference No (Customer) ── */}
            <div className="flex flex-col gap-[0.3vw]">
              <label className="font-semibold text-gray-600">
                Reference No <span className="text-gray-400 font-normal">(Customer)</span>
              </label>
              <input
                value={base.refNoCustomer}
                onChange={e => sb("refNoCustomer", e.target.value)}
                placeholder="Enter customer ref no…"
                className="border border-gray-300 p-[0.6vw] rounded-[0.4vw] outline-none focus:ring-2 ring-blue-100 focus:border-blue-400"
              />
            </div>

            {/* ── NEW: Reference No (Internal) ── */}
            <div className="flex flex-col gap-[0.3vw]">
              <label className="font-semibold text-gray-600">
                Reference No <span className="text-gray-400 font-normal">(Internal)</span>
              </label>
              <input
      ...
      <table className="w-full text-left border-collapse">
        <thead className="bg-blue-50 sticky top-0 z-10 shadow-sm">
          <tr>
            ...
            {["S.No", "Ref No", "Date", "Customer", "Product", "Serial No", "Nature", "Warranty", "Ref (Customer)", "Ref (Internal)", "Assigned To", "Esc. Level", "Expected Close", "Status", ""].map(h => (
          ...
          {paginatedData.length > 0 ? paginatedData.map((row, i) => {
            ...
            return (
              <tr key={row.id} className={`transition-colors ${isSelected ? "bg-blue-50" : "hover:bg-gray-50"}`}>
                ...
                {/* Ref (Customer) */}
                <td className="p-[0.8vw] border-r border-gray-200 text-[0.75vw] text-gray-600 font-mono">{row.refNoCustomer || "—"}</td>
                {/* Ref (Internal) */}
                <td className="p-[0.8vw] border-r border-gray-200 text-[0.75vw] text-gray-600 font-mono">{row.refNoInternal || "—"}</td>
                <td className="p-[0.8vw] border-r border-gray-200">
                  {row.assignedToName
            ...
            <tr><td colSpan={16} className="py-[4vw] text-center text-gray-400">No records found.</td></tr>
          )}
        </tbody>
      </table>
    </div>
...
export default function ServiceMaterialInward() {
  ...
  const filteredData = useMemo(() => {
    ...
    return entries.filter(row => {
      const ms = !s || row.refNo?.toLowerCase().includes(s) || row.customerName?.toLowerCase().includes(s) || row.productDescription?.toLowerCase().includes(s) || row.assignedToName?.toLowerCase().includes(s) || row.refNoCustomer?.toLowerCase().includes(s) || row.refNoInternal?.toLowerCase().includes(s);
