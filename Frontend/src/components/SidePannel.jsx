...
export default function Sidebar() {
  ...
  const navItems = [
    {
      path: "/dashboard",
      label: "Dashboard",
      icon: "/SidePannelLogos/Dashboard.svg",
      show: true,
    },
    {
      path: "/masterPage",
      label: "Master Access",
      icon: "/SidePannelLogos/access.png",
      show: isAdmin,
    },
    {
      path: "/customers",
      label: "Customers",
      icon: "/SidePannelLogos/Activity.svg",
      show: isAdmin,
    },
    {
      path: "/serviceCall",
      label: "Service Call",
      icon: "/SidePannelLogos/call.png",
      show: isAdmin,
    },
    {
      path: "/serviceMaterial",
      label: "Service Material",
      icon: "/SidePannelLogos/service.png",
      show: isAdmin,
    },
    {
      path: "/productionMaterial",
      label: "Production Material",
      icon: "/SidePannelLogos/production.png",
      show: isAdmin,
    },
    {
...

    {
      path: "/troubleshoot",
      label: "Troubleshoot",
      icon: "/SidePannelLogos/Messages.svg",
      show: true,
    },
  ];
  ...
  return (
    ...
    >
      {/* Logo Section */}
      <div
        className={`flex items-center justify-center border-b border-gray-50 transition-all duration-500 ease-out ${isCollapsed ? "h-[10vh] px-[0.5vw]" : "h-[12vh] px-[1vw]"}`}
      >
        <img
          src="/Akira_logo.webp"
          alt="Logo"
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className={`relative flex-shrink-0 flex items-center justify-center transition-all duration-300 ${isCollapsed ? "w-[1.8vw] h-[1.8vw]" : "w-[1.4vw] h-[1.4vw]"}`}
                    >
                      <img
                        src={item.icon}
                        alt={item.label}
                    ...
                    <span
                      className={`whitespace-nowrap font-medium text-[0.85vw] transition-all duration-500 ease-out ${isCollapsed ? "w-0 opacity-0 translate-x-[-1vw] pointer-events-none" : "w-auto opacity-100 translate-x-0"}`}
                    >
                      {item.label}
                    </span>
                    {active && (
                      <div className="absolute left-0 top-[15%] bottom-[15%] w-[0.25vw] bg-white rounded-r-full animate-slideIn" />
                    )}
                  </Link>
      ...
      <div
        className={`py-[1.2vh] transition-all duration-500 ease-out ${isCollapsed ? "px-[0.4vw]" : "px-[0.8vw]"}`}
      >
