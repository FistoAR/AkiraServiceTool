...
function checkEscalations(data) {
  ...
  const updatedQueue = escalationQueue.map((entry) => {
    ...
    if (now >= deadline) {
      ...
      if (nextLevel < ESCALATION_FLOW.length) {
        const nextDepartment = ESCALATION_FLOW[nextLevel];
        ...
        if (candidates.length > 0) {
          ...
          const escalationRecord = {
            ...
            status: "Pending",
            previousDepartment: ESCALATION_FLOW[currentLevel],
            previousEngineerId: entry.currentEngineerId,
            previousEngineerName: entry.currentEngineerName,
            reason: `No response within 20 minutes from ${ESCALATION_FLOW[currentLevel]}`,
          };
          ...
          entry.deadline = escalationRecord.deadline;
          ...
          notifications.push({
            ...
            from: ESCALATION_FLOW[currentLevel],
            to: nextDepartment,
            engineer: selectedEngineer.name,
            engineerId: selectedEngineer.userId,
            message: `Call ${entry.callNumber} escalated from ${ESCALATION_FLOW[currentLevel]} to ${nextDepartment} (${selectedEngineer.name})`,
